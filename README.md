...
- **`@dsrv/wallet-core`**: Core types and interfaces (no runtime dependencies)
- **`@dsrv/wallet-keys-aws`**: AWS Secrets Manager implementation for mnemonic storage
- **`@dsrv/wallet-eth`**: Ethereum wallet adapter using ethers v6
...
import { GenerateAddressParams, UnsignedTx } from '@dsrv/wallet-core';
...
import { AwsMnemonicProvider } from '@dsrv/wallet-keys-aws';
import { EthWalletAdapter } from '@dsrv/wallet-eth';
import { CoinType, WalletAdapter } from '@dsrv/wallet-core';
...
1. Create a new adapter package (e.g., `@dsrv/wallet-trx`, `@dsrv/wallet-sol`)
2. Implement the `WalletAdapter` interface from `@dsrv/wallet-core`
...
```typescript
import { WalletAdapter, CoinType } from '@dsrv/wallet-core';
